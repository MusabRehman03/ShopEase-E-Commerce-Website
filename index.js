...
function displayData(array = arr) {
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      //   console.log("data=>", array[button.getAttribute("index")]);
...
function displayCartData(array = cartArray) {
  ...
  div.innerHTML = array
    .map((element, index) => {
      ...
      totalNumberOfProductsInCart += cartCountArray[n];
      ...
      return `<div  class=" productCard p-4 border rounded-lg shadow-md h-fit w-80 flex flex-col justify-center items-center">
              ...
              <button  index=${index} onclick="decrement(${n})" class=" plus  mt-2 px-4 py-2 hover:bg-red-500 bg-red-400 text-white rounded">-</button>
              <span class="mt-2 px-4 py-2" id="count">${cartCountArray[n]}</span>
            <button index=${index} onclick="increment(${n})" class=" minus mt-2 px-4 py-2 hover:bg-blue-500 bg-blue-400 text-white rounded">+</button>
  ...
  document.getElementById(
    "checkOut"
  ).innerText = `Check Out(${totalNumberOfProductsInCart})`;
}
...
function confirmPayment() {
  ...
  c.innerHTML = cartArray
    .map(
      (
        element
      ) => `<div class="flex justify-between"><h2 class="text-lg font-semibold">${
        element.title
      }</h2>
                
                <p class="text-green-500 font-bold">$${
                  element.price * cartCountArray[Number(element.id) - 1]
                }</p>
                </div>
    `
    )
